...
The Vgccoin Core daemon can be configured to act as a trusted "border
router", implementing the vgccoin wire protocol and relay, making
...
The ZeroMQ feature in Vgccoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vgccoind:
...
For instance:
    ...
    $ vgccoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/vgccoind.tx.raw \
...
These options can also be provided in vgccoin.conf.
...
From the perspective of vgccoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vgccoind directly. Furthermore, no information is
...
using. Vgccoind appends an up-counting sequence number to each
