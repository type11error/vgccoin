    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional VGC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes VGC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::VGC_TO_VGC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a VGC-to-VGC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::VGC_ONLY;
        ...
        // If VGC-to-VGC fails, create a peg-out transaction
