...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s vgccoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "vgccoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `vgccoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
